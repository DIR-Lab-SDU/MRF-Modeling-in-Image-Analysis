...
function αexpansion{T<:Number}(
    ...
    I::Vector{T},                       # fixed image
    J::Vector{T},                       # moving image
    ...
    while successFlag == 1 && count <= 100   # 4. If success=1 goto 2
        ...
        for α in L
            ...
            for p in Partition.vertices
                ...
                # t{p}{α}, p ∈ P
                capacityMatrix[nodeα,p] = D(I, J, α, p)
                if f[p] == α
                    # add edges to ᾱ
                    ...
                    capacityMatrix[p,nodeᾱ] = D(I, J, f[p], p)
                end
            end
            for p in Partition.vertices, q in neighborhood[p]
                if f[p] == f[q]
...
# load data
using EMPIRE10
fixedData = EMPIRE10.load("/Users/gnimuc/Documents/4DCTdata/scans/01_Fixed.raw")
movingData = EMPIRE10.load("/Users/gnimuc/Documents/4DCTdata/scans/01_Moving.raw")
# load images
fixedImage, coronal, sagittal = EMPIRE10.loadimages(fixedData, 120, 1, 1)
movingImage, coronal, sagittal = EMPIRE10.loadimages(movingData, 120, 1, 1)
# view(fixedImage)
# view(movingImage)

# load pixels
fixed, b, c = EMPIRE10.loadpixels(fixedData, 120, 1, 1)
moving, c, b = EMPIRE10.loadpixels(movingData, 120, 1, 1)
# normalized 0~255
fixed = fixed - minimum(fixed)
fixed = fixed / maximum(fixed)
fixed = round(fixed * 255)

moving = moving - minimum(moving)
moving = moving / maximum(moving)
moving = round(moving * 255)

# registering
ω = Float64[-15:15;]

function smoothterm(Dp, Dq)
    return 0.05*255*abs(Dp-Dq)
end

function dataterm(I, J, D, p)
    return abs(I[p]-J[p+D[p]])
end

# setup neighborhood system
n6 = [1,7,11]
m7 = [2,6,8,12]
...
# get deformable field
len = length(fixed)
@show size(fixed)
defeild = αexpansion(ω, dataterm, reshape(fixed, len), reshape(moving, len), smoothterm, neighborhood)
