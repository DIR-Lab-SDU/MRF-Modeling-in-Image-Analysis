...
function αexpansion(
    ...
    I::Vector,                       # observed intensity
    V::Function,                        # smooth term
    neighborhood::Vector   # adjacency list
    )
...
labelSet = collect(1:labelNum) - 1

function dataFn(pixel, label)
    if pixel < 25
        return label == 0 ? 0 : 10
    else
        return label == 5 ? 0 : 10
...
smoothFn(l1, l2) = (l1-l2)*(l1-l2) <= 4 ? (l1-l2)*(l1-l2) : 4
...
image = rand(50)
out = αexpansion(labelSet, dataFn, image, smoothFn, neighborhood)
@show out
# exports
